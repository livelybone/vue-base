<template>
  <div class="textarea-wrap">
    ...
    <span v-if="config.maxLength" class="count">
      <span :class="myValue.length<config.maxLength?'font-blue':'font-red'">{{myValue.length}}</span>/{{config.maxLength}}
    </span>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      input(val) {
        this.myValue = val;
        this.pristine = false;
        this.valid = (val && this.config.validateFn && this.config.validateFn(val)) || (this.config.required === false && !val);
